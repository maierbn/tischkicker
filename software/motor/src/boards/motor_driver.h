...
private:
  ...
  bool parseResponse(unsigned char byte, RegisterAddress registerAddress, bool printOutput=true);

  ///! write 2 bytes to std output
  void output(std::string specifier, unsigned char byte[2]);

  ///! read the given register from the device and store the information
  unsigned char readRegister(RegisterAddress registerAddress, bool printOutput=true);
  
  ///! write the given value to the specified register on the device
  ...
  ///! read the current microStepPosition from the device
  void readPosition();
