...
public:
  ...
  {
    CSActive = 0,     // line at chip output is 0 => ¬CS = 0 (low-active)
    CSInactive = 1    // line at chip output is 1 => ¬CS = 1 (low-active)
  };
...
private:
  ...
  //! configure hid_read to be blocking and receive all data that was triggered by previous hid_write without hid_reads (on non-blocking hid_reads that did not finish)
  void readBlocking(hid_device *hidDevice, unsigned char *buffer, int length);
  ...
  chipUSBSettings;
	...
	unsigned char pending_command_id_;
};
